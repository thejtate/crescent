(function ($) {
  ...
  function initTopMenu() {
    ...
    var $btn = $('.hamburger-wrapper');

    if ($btn.hasClass('processed')) return;
  ...
  function initTabslogo() {

    if ($(window).outerWidth() > 980) return;

    var $wrapper = $('#block-views-fblog-category-block');

    var $list = $wrapper.find('.view-fblog-category'),
      $nav = $list.find('.view-content');

    var text = ($list.find('.views-summary a.active').length) ? $list.find('.views-summary a.active').text() : 'Сhoose';
    $list.prepend('<span class="title-nav">' + text + '</span>');
    ...
    $nav.find('a').on('click touch', function () {
      $list.removeClass('active');
      $nav.slideUp();
      $title.html($(this).clone());
    })
  ...
  function initTabsBg() {
    ...
    if ($(window).outerWidth() < 980) {
      return;
    } else if ($('body').hasClass('page-healthsafety')) {
      $wrapper.parent().prepend('<span style="height: ' + (height + 20) + 'px;" class="bg"></span>');
    } else {
  ...
  function initContactTabs() {
    ...
    function checkTarget(e) {
      ...
      if ($target.hasClass(config.titleClassName)) {
        ...
        showFullListOfStatesButton($target);
        ...
        hideFullListOfStatesButton($target);
    ...
    function hideFullListOfStatesButton(el) {
      el.parents('.link-show-all').hide();
    }

    function showFullListOfStatesButton(el) {
      var $parent = el.closest('.control-wrapper');
      var link = $parent.find('.link-show-all');
      link.show();
    }
  }
